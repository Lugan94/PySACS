...
from .base_functions import compute_strains, compute_forces, compute_moment, find_equilibrium, axial_residual
import numpy as np
...
class MomentCurvatureAnalysis():
    ...
    def run(self, phi_max: float, n_steps: int, axial_load: float = 0, angle: float = 0) -> MomentCurvatureResults:
        ...
        phi_array[0] = 1e-6
        z_coords = self.rotate_coords(angle)[:,1]
        ...
        for phi in phi_array:
            try:
                print(f"residual en a: {axial_residual(strain_limits[0], phi, z_coords, self.areas, self.models, 0)}")
                print(f"residual en b: {axial_residual(strain_limits[1], phi, z_coords, self.areas, self.models, 0)}")
                ...
                print(f"el momento es: {moment}")

            except:
                print(f"La sección ha fallado por falta de equilibrio en phi = {phi:.6f}")
                # Cortamos el array de phi para que coincida con los momentos calculados
                phi_array = phi_array[:len(moments)]
                break
